...
To sum up, we have seen that not only does the NMF generate meaningful features, it has also many applications in several domains. Indeed, we can use nonnegative matrices to describe very different things, from pixel values, to word counts, graphs, linear optimization problems, but we can also find them in probability, communication complexity and many more. And although there can be some difficulties in this linear dimensionality reduction, that it is NP-hard and ill-posed, there still exists different ways to deal with them.
