...
% TODO
% - copy the statements (?)
% - clean up all notations
% - check A2 group q with TA
...
Finally, the set \(\field^{n \times n}\) equipped with the Kronecker product is not a group, as one can easily see that the Kronecker product of two matrices in \(\field^{n \times n}\) is an element of \(\field^{n^2 \times n^2}\).
...
\]
The second equality is valid since \(A\) and \(-A\) commute.
It is then straightforward that $\rank(e^A)=n$.
\end{proof}
