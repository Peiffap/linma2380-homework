...
\subsection*{A1}
Minimising $\snorm{Ax - b}$ in respect to x means finding x such that the derivative of $\snorm{Ax - b}$ in respect to x is equal to zero.\\
$\snorm{Ax - b}$ can be developed as: \\
\begin{align*}
    \snorm{Ax - b} &= \snorm{Ax} - 2 <Ax,b> + \snorm{b} \\
    &= x^{T}A^{T}Ax - 2x^{T}A^{T}b + b^{T}b 
\end{align*}
The derivative in respect to x is: \\
\begin{align*}
    \frac{\partial \snorm{Ax - b}}{\partial x} &= \frac{\partial(x^{T}A^{T}Ax - 2x^{T}A^{T}b + b^{T}b)}{\partial x} \\
...
\begin{align*}
    \frac{\partial \snorm{Ax - b}}{\partial x} = 0 \\
    \Leftrightarrow 2A^{T}Ax - 2A^{T}b = 0 \\
    \Leftrightarrow 2A^{T}Ax = 2A^{T}b \\
    \Leftrightarrow A^{T}Ax = A^{T}b
\end{align*}

To demonstrate that when A has full rank column, i.e. rank(A)=n, the solution is unique, it is needed to prove that Ker($A^{T}A$)=Ker(A). \\
\begin{align*}
    \forall x \in Ker(A): Ax = 0 \Leftrightarrow A^{T}Ax = 0 \Leftrightarrow x \in Ker(A^{T}A) \Rightarrow Ker(A) \subset Ker(A^{T}A)
\end{align*}

\begin{align*}
    \forall x \in Ker(A^{T}A): A^{T}Ax = 0 \Leftrightarrow x{T}A^{T}Ax = 0 \Leftrightarrow (Ax)^{T}(Ax) = 0 \Leftrightarrow Ax = 0 \Leftrightarrow x \in Ker(A) \\
...
\end{align*}
proving that the solution is unique according to Theorem 1.2. seen in the course.
